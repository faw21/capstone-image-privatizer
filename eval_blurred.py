...
if __name__ == '__main__':
    ...
    parser.add_argument('--upsample', type=str, default='2')
    parser.add_argument('--times', type=int, default=1)
    parser.add_argument('--outputdir', type=str, default='./step3/')
    ...
    for k in range(0, args.times):
        for j in range(1,21):
            start_time = time.time()
            image_dir = args.coco_dir + 'face_blurred_upsample_%s_radius_%s/' % (args.upsample, str(j))
            ...
            write_json = args.outputdir + '%s_%s_%s.json' % (args.resize, args.upsample, str(j))
            write_csv = args.outputdir + 'estimation_result_%s_%s_%s.csv' % (args.resize, args.upsample, str(j))
