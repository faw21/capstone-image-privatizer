...
if __name__ == '__main__':
    ...
    parser.add_argument('--resize', type=str, default='496x496', help='if provided, resize images before they are processed. default=0x0, Recommends : 432x368 or 656x368 or 1312x736 ')
    ...
    parser.add_argument('--coco-dir', type=str, default='./COCODatasets/')
    parser.add_argument('--data-idx', type=int, default=-1)
    parser.add_argument('--multi-scale', type=bool, default=False)
    parser.add_argument('--times', type=int, default=1)
    parser.add_argument('--outputdir', type=str, default='./step1/')
    ...
    for j in range(0, args.times):
        ...
        write_json = args.outputdir + '%s_coco%s_%s.json' % (args.model, args.cocoyear, args.resize)
        write_csv = args.outputdir + 'estimation_result_%s_coco%s_%s.csv' % (args.model, args.cocoyear, args.resize)
        ...
        with open(write_csv, 'a') as csvfile:
            ...
            writer.writerow(list(cocoEval.stats) + [total_time,])
